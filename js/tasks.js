...
const getUsersWithEyeColor = (users, color) => {
  const getUsersWithBlueEyes = users.filter(user => user.eyeColor === color);
  //    .map(user => user.name);
  return getUsersWithBlueEyes;
};
...
const getUsersWithAge = (users, min, max) => {
  let userWithAge = users.filter(user => user.age > min && user.age < max);
  return userWithAge;
};
